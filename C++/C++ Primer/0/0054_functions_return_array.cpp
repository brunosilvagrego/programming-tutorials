...
int *p1[10];            // p1 is an array of ten pointers to int
...
int main()
{
    ...
    {
        cout << p[i] << " ";        // p points to an element in an array, which we subscript
    }
	...
	for (size_t i = 0; i < 5; ++i)
    {
        cout << (*arrP)[i] << " ";  // arrP points to an array, we must dereference the pointer to get the array itself
    }
	...
	for (size_t i = 0; i < 5; ++i)
    {
        cout << arrR[i] << " ";     // arrR refers to an array, which we can subscript
    }
